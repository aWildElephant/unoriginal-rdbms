...
import static fr.awildelephant.rdbms.ast.builder.WithBuilder.with;
...
class WithParserTest {
    ...
    void it_should_parse_a_with_statement_with_a_single_element() {
        assertParsing("WITH with_query AS (SELECT a, b FROM t1) SELECT * FROM with_query",
                with().with("with_query", select().outputColumns(unqualifiedColumnName("a"), unqualifiedColumnName("b")).fromClause(tableName("t1")))
                        .query(select().outputColumns(asterisk())
                                .fromClause(tableName("with_query"))));
    }
    ...
    void it_should_parse_a_with_statement_with_several_elements() {
        assertParsing("WITH first_clause AS (select * from t1), second_clause AS (select * from t2), third_clause AS (select * from t3) select * from t1, t2, t3",
                with().with("first_clause", select()
                                .outputColumns(asterisk())
                                .fromClause(tableName("t1")))
                        .with("second_clause", select()
                                .outputColumns(asterisk())
                                .fromClause(tableName("t2")))
                        .with("third_clause", select()
                                .outputColumns(asterisk())
                                .fromClause(tableName("t3")))
                        .query(select()
                                .outputColumns(asterisk())
                                .fromClause(tableReferenceList(tableName("t1"), tableName("t2"), List.of(tableName("t3"))))));
    }
}
