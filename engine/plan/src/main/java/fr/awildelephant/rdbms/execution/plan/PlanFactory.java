...
public final class PlanFactory extends DefaultLopVisitor<String> {

    private final Set<PlanStep> steps = new HashSet<>();
    ...
    public static Plan build(LogicalOperator logicalPlan) {
        final PlanFactory builder = new PlanFactory();
        final String target = builder.apply(logicalPlan);
        return new Plan(builder.steps, target);
    }
