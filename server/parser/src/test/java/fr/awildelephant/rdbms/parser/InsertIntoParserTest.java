...
class InsertIntoParserTest {
    ...
    void it_should_parse_an_insert_into_statement_with_a_single_column_and_a_single_row() {
        assertParsing("INSERT INTO test VALUES (1)", insertInto(tableName("test"), rows(row(integerLiteral(1)))));
    }

    @Test
    void it_should_parse_an_insert_statement_with_a_single_column_and_several_rows() {
        assertParsing("INSERT INTO test VALUES (1), (2)", insertInto(tableName("test"), rows(row(integerLiteral(1)), row(integerLiteral(2)))));
    }

    @Test
    void it_should_parse_an_insert_into_statement_with_several_columns_and_a_single_row() {
        assertParsing("INSERT INTO test VALUES (1, 2, 3)", insertInto(tableName("test"), rows(row(integerLiteral(1), integerLiteral(2), integerLiteral(3)))));
    }

    @Test
    void it_should_parse_an_insert_into_statement_with_a_text_literal() {
        assertParsing("INSERT INTO test VALUES ('some text')", insertInto(tableName("test"), rows(row(textLiteral("some text")))));
    }

    @Test
    void it_should_parse_an_insert_into_statement_with_a_decimal_literal() {
        assertParsing("INSERT INTO test VALUES (19.99)", insertInto(tableName("test"), rows(row(decimalLiteral(new BigDecimal("19.99"))))));
    }

    @Test
    void it_should_parse_an_insert_into_statement_with_the_null_value() {
        assertParsing("INSERT INTO test VALUES (null)", insertInto(tableName("test"), rows(row(nullLiteral()))));
    }

    @Test
    void it_should_parse_an_insert_into_statement_with_a_date_literal() {
        assertParsing("INSERT INTO test VALUES (date '2018-07-15')", insertInto(tableName("test"), rows(row(cast(textLiteral("2018-07-15"), DATE)))));
    }
}
